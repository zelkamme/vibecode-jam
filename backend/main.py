...
from models import User, Question, Report, TestSession, Vacancy, UserAnswer

from llm.qa_gen import generate_theory_qa, generate_theory_check
from llm.helper_ai import generate_helper_ai  

from llm.code_review import generate_code_review
from llm.unit_tests_gen import generate_unittests
...
async def lifespan(app: FastAPI):
    ...
    try:
        seed_questions()
        print("База данных вопросов проверена/обновлена.")
    except Exception as e:
        print(f"Ошибка при авто-заполнении вопросов: {e}")
    yield
...
def theory_start(data: TheoryStartRequest, session: Session = Depends(get_session)):
    # 1. Определяем целевой язык пользователя по Вакансии
    target_tag = "python" # Дефолт
    
    user = session.get(User, data.user_id)
    if user and user.vacancy_id:
        vac = session.get(Vacancy, user.vacancy_id)
        if vac:
            # Превращаем "JavaScript" -> "javascript", "Python" -> "python"
            target_tag = vac.language.lower()
            print(f"🎯 Пользователь {user.username}: Вакансия {vac.title}, язык {target_tag}")

    # 2. Ищем вопрос, совпадающий по УРОВНЮ и ЯЗЫКУ (тегу)
    # Используем contains, чтобы искать подстроку (на случай "python,django")
    q = session.exec(
        select(Question)
        .where(Question.type == "theory")
        .where(Question.level == data.level)
        .where(Question.required_tag.contains(target_tag)) # <--- ФИЛЬТР ПО ЯЗЫКУ
        .limit(1)
    ).first()
    
    # Фолбек 1: Если вопросов уровня Junior нет, ищем любые вопросы этого языка
    if not q:
         q = session.exec(
            select(Question)
            .where(Question.type == "theory")
    ...
    # Фолбек 2: Если вообще ничего не нашли (например, редкий язык), берем любой вопрос
    if not q:
    ...
    # 3. Ищем или создаем сессию (код без изменений)
    ...
    theory_session_state[session_key] = {
        ...
        "topic_attempts": 0,
        "target_tag": target_tag  # <--- ЗАПОМИНАЕМ ЯЗЫК В СЕССИИ
    }

    parts = q.text.split("\n\n", 1)
    formatted_question = f"**Вопрос 1 ({target_tag.upper()}): {parts[0]}**\n\n{parts[1] if len(parts) > 1 else ''}"
...
def theory_chat(data: TheoryChatMessage, session: Session = Depends(get_session)):
    ...
    if follow_up and follow_up != "NEXT_QUESTION" and state["topic_attempts"] == 0:
        ...
        target_tag = state.get("target_tag", "python") # Достаем язык из памяти
        
        # Ищем следующий вопрос того же языка
        next_q = session.exec(
            select(Question)
            .where(Question.type == "theory")
            .where(Question.required_tag.contains(target_tag)) # <--- ФИЛЬТР
            .offset(state["questions_asked"] - 1)
            .limit(1)
        ).first()
        
        # Если вопросы этого языка закончились, берем любой  
        if not next_q:
        ...
        ai_message = f"**Вопрос {state['questions_asked']} ({target_tag.upper()}): {parts[0]}**\n\n{parts[1] if len(parts) > 1 else ''}"
...
LANGUAGE_CONFIG = {
    "Python": {
        ...
        "file_name": "main.py",
        "command": ["python", "main.py"]  # Убрали /work/
    },
    "JavaScript": {
    ...
    "C++": {
        "image": "gcc:latest",
        "file_name": "main.cpp",
        "command": ["sh", "-c", "g++ -o app main.cpp && ./app"]
    },
    "Go": {
...
# Путь внутри контейнера backend, куда примонтирован volume
SHARED_VOL_PATH = "/tmp/shared"
os.makedirs(SHARED_VOL_PATH, exist_ok=True)
    ...
    if payload.user_id:
        ...
        if user and user.vacancy_id:
            ...
            for key in LANGUAGE_CONFIG.keys():
                if vac.language.lower() in key.lower():
                    target_lang = key
                    break
    
    if payload.language and payload.language in LANGUAGE_CONFIG:
        target_lang = payload.language

    config = LANGUAGE_CONFIG.get(target_lang, LANGUAGE_CONFIG["Python"])
    image_to_run = config["image"]
    file_name = config["file_name"]
    run_command = config["command"]

    # --- ИСПРАВЛЕНИЕ: ИСПОЛЬЗУЕМ ОБЩИЙ VOLUME ---
    
    # 1. Генерируем уникальную подпапку в общем томе
    # Убедитесь, что SHARED_VOL_PATH существует
    os.makedirs(SHARED_VOL_PATH, exist_ok=True)
    
    run_id = str(uuid.uuid4())
    host_run_dir = os.path.join(SHARED_VOL_PATH, run_id)
    os.makedirs(host_run_dir, exist_ok=True)

    # 2. Записываем файл кода
    source_path = os.path.join(host_run_dir, file_name)
    with open(source_path, "w", encoding="utf-8") as f:
        f.write(payload.code)

    try:
        # 3. Запускаем Docker с использованием Named Volume
        # Мы монтируем том 'code_runner_shared' в папку /task_root внутри раннера.
        # Docker сам найдет файлы, которые backend положил в этот том.
        
        container = docker_client.containers.run(
            image=image_to_run,
            command=run_command,
            volumes={
                "code_runner_shared": {"bind": "/task_root", "mode": "rw"}
            },
            working_dir=f"/task_root/{run_id}", # Рабочая папка — наша уникальная директория
