from llm.llm_api import common_list_parser
...
def generate_theory_qa(position, requirements, resume, llm_api, redis_host="redis", redis_port=6379):
...
def generate_code_qa(position, requirements, resume, code, llm_api, redis_host="redis", redis_port=6379):
...
def generate_theory_check(question, ideal_answer, user_answer, llm_api, redis_host="redis", redis_port=6379):
...
def test_code_qa(llm_api, redis_host="redis", redis_port=6379):
...
def test_theory_qa(llm_api, redis_host="redis", redis_port=6379):
    position = "Junior Python Developer"
    requirements = "ООП, REST API, Django, PostgreSQL, Git, базовые алгоритмы, написание unit‑тестов"
    resume = """Студент 4‑го курса ИТ‑специальности. 1 год практики в Python. Реализовал небольшое веб‑приложение на Flask, знаком с Git, проходил курс «Введение в Django», написал несколько скриптов для автоматизации.
    """
...
def test_theory_check(llm_api, redis_host="redis", redis_port=6379):
