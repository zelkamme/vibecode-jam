...
def generate_code_review(lang, question, ideal_answer, user_answer, position, llm_api, redis_host="redis", redis_port=6379):
...
def test_code_review(llm_api, redis_host="redis", redis_port=6379):
    lang = "Python"
...
def generate_lang_detect(code, llm_api, redis_host="redis", redis_port=6379):
    """Детект ЯП:
    Вход: код
    Выход: язык
    """
...
def test_lang_detect(llm_api, redis_host="redis", redis_port=6379):
    code = """
    def unique_preserve_order(lst):
        seen = set()
        return [x for x in lst if not (x in seen or seen.add(x))]
